...
Persistence is Drizzle ORM 0.45.x using its built-in `drizzle-orm/expo-sqlite` driver — the only ORM that can run on-device in React Native. `drizzle-kit` generates migrations (`dialect: sqlite, driver: expo`), bundled into the app via `babel-plugin-inline-import` and applied at startup with `useMigrations`. The schema lives in a single `db/schema.ts`; a `getDb()` seam returns the expo driver in the app and better-sqlite3 (dev-only) in tests, so one schema serves both.
