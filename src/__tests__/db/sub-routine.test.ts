...
describe('sub-routine binding — Routine × Gym maps Movement → Equipment class', () => {
  ...
  test('bindings are independent per Gym — the differentiator holds', async () => {
    ...
    expect(
      (await getSubRoutine(db, { routineId: routine.id, gymId: gymA.id }))?.entries[0]
        .equipmentClass,
    ).toBe('barbell');
    expect(
      (await getSubRoutine(db, { routineId: routine.id, gymId: gymB.id }))?.entries[0]
        .equipmentClass,
    ).toBe('dumbbell');
  });
  ...
  test('each Routine × Gym has exactly one sub-routine', async () => {
    ...
    const rows = await db
      ...
      .where(
        and(eq(schema.subRoutines.routineId, routine.id), eq(schema.subRoutines.gymId, gym.id)),
      );
