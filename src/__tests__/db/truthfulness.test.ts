...
describe('truthfulness — recording scale and canonical loads', () => {
  ...
  async function oneSetSession(
    gymId: number,
    set: { loadLb: number; reps?: number; proximity?: { scale: string; value: number } },
  ) {
  ...
  test('proximity values carry the Recording scale they were written in', async () => {
    ...
    const [set] = await oneSetSession(gym.id, {
      loadLb: 100,
      proximity: { scale: 'rpe', value: 1 },
    });
  ...
  test('mixing Recording scales within one island keeps each value truthful', async () => {
    ...
    const sets = await setsByIsland(db, {
      gymId: gym.id,
  ...
  test('loads persist canonically in pounds, whatever the movement unit', async () => {
    ...
    for (const entryId of [kgSet.sessionEntryId, lbSession.entry.id]) {
      const row = (
        await db.select().from(schema.sets).where(eq(schema.sets.sessionEntryId, entryId))
      )[0];
      expect(row.loadLb).toBe(100);
      expect(Object.keys(row)).not.toContain('unit');
    }

    const kgIsland = await setsByIsland(db, {
      gymId: gym.id,
      movementId: inKg.id,
      equipmentClass: 'barbell',
    });
    const lbIsland = await setsByIsland(db, {
      gymId: gym.id,
      movementId: inLb.id,
      equipmentClass: 'barbell',
    });
