...
describe('catalog browse read path', () => {
  ...
  test('user-created movements join the same A–Z list and retain Instructions', async () => {
    ...
    expect(rows.find((r) => r.name === 'Instructional Press')?.instructions).toBe(
  ...
  test('archive and unarchive preserve history while changing picker visibility', async () => {
    ...
    await db
      ...
      .values({ routineId: routine.id, position: 0, movementId: movement.id });
    ...
    expect(
      (await listMovements(db, { includeArchived: true, query: movement.name }))[0].archived,
    ).toBe(true);
  ...
  test('deletes a movement with no history or references', async () => {
    ...
    expect(
      await db.select().from(schema.movements).where(eq(schema.movements.id, movement.id)),
    ).toEqual([]);
  });
  ...
  test('blocks deletion for history and groups its session reference', async () => {
    ...
    const gym = await makeGym(db, 'Downtown');
    const { session, entry } = await startSession(db, {
      gymId: gym.id,
      movement,
    ...
    expect(references.sessions).toEqual([
      { id: session.id, name: 'Downtown', date: session.startedAt },
    ]);
