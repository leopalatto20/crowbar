...
describe('routine builder state seam', () => {
  test('configures targets before insertion and keeps them editable after insertion', () => {
    ...
    const rirDraft = updateRoutineEntryTarget(
      createRoutineDraft('Push', 'rir', [{ movementId: 1 }]),
      1,
      'proximityValue',
      '0',
    );
  ...
  test('reports invalid target values and independently clears fields', () => {
    ...
    expect(invalid.entryErrors[1]).toEqual(
    ...
    expect(validateRoutineDraft(draft).entryErrors[1]).not.toContain(
      'RIR target must be between 0 and 10.',
    );
  });
  ...
  test('parses create and edit as route state and rejects incomplete builder routes', () => {
    ...
    expect(parseRoutineBuilderRoute({ builder: 'edit', routineId: '12' })).toEqual({
      mode: 'edit',
      routineId: 12,
    });
  ...
  test('only prompts for persisted edits or unfinished interruptions', () => {
    ...
    expect(
      isRoutineDraftDirty(
        { ...initial, lastRemoved: { entry: initial.entries[0], index: 0 } },
        initial,
      ),
    ).toBe(false);
    ...
    quickCreate = quickCreateReducer(quickCreate, {
    ...
    quickCreate = quickCreateReducer(quickCreate, {
      type: 'movementSaved',
      movement: { id: 4, name: 'New movement' },
    });
    expect(isQuickCreateDraftDirty(quickCreate, 'kg')).toBe(false);
  });
});
