...
export type RoutineEntryTargetField =
  'workingSetCount' | 'repMin' | 'repMax' | 'proximityValue' | 'tempo';
...
export function undoRoutineEntryRemoval(draft: RoutineDraft): RoutineDraft {
  if (
    !draft.lastRemoved ||
    draft.entries.some((entry) => entry.movementId === draft.lastRemoved?.entry.movementId)
  ) {
    return draft;
  }
...
export function validateRoutineDraft(draft: RoutineDraft): RoutineDraftValidation {
  ...
  for (const entry of draft.entries) {
    ...
    if (seenMovementIds.has(entry.movementId))
      errors.push('A Movement can only appear once in a Routine.');
