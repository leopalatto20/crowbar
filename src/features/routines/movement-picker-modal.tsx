          ...
          <Box className="h-[86%] rounded-t-xl bg-background px-4 pb-4 pt-4 web:mx-auto web:w-full web:max-w-[800px]">
            <Box className="min-h-0 flex-1 gap-4">
              ...
              <Button
                ...
                accessibilityLabel="Create movement"
              >
                <ButtonText>
                  {query.trim() ? `Create “${query.trim()}”` : 'Create movement'}
                </ButtonText>
              </Button>
              <ScrollView
                horizontal
                  ...
                  hasFilters ? (
                    <Box className="items-center gap-4 rounded-xl bg-card px-4 py-8">
                      ...
                      <Button
                        ...
                        accessibilityLabel="Clear movement filters"
                      >
                    ...
                    <Box className="items-center gap-4 rounded-xl bg-card px-4 py-8">
                      <Text className="text-center text-muted-foreground">
                        There are no active movements. Create one above or add one in Catalog.
                      </Text>
                      <Button
                        variant="outline"
                        onPress={onClose}
                        accessibilityLabel="Close picker to add a movement in Catalog"
                      >
                        ...
                        <Box className="flex-row items-start gap-2">
                          ...
                          {alreadyAdded && (
                            <Button
                              ...
                              size="sm"
                              className="px-0"
                              onPress={() => onRevealExisting(movement.id)}
                              accessibilityLabel={`Reveal ${movement.name} in ledger`}
                            >
                        ...
                        {hasInstructions && (
                          <>
                            <Button
                              ...
                              onPress={() => toggleInstructions(movement.id)}
                              accessibilityLabel={`${instructionsVisible ? 'Hide' : 'Show'} instructions for ${movement.name}`}
                            >
                              <ButtonText>
                            ...
                            {instructionsVisible && (
                              <Text className="text-muted-foreground">{movement.instructions}</Text>
                            )}
                          </>
                        )}
                      </Box>
