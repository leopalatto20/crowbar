  ...
  return (
    <Box className={`rounded-xl bg-card ${routine.archived ? 'opacity-50' : ''}`}>
      <Box className="flex-row items-center gap-2 px-4 py-4">
        ...
        >
          <ButtonText
            className="min-w-0 flex-1 justify-start text-left text-base"
            numberOfLines={1}
          >
            {routine.name}
          </ButtonText>
        ...
        <Text size="sm" className="text-muted-foreground">
          {movementCountLabel(routine.movementCount)}
        </Text>
        <Button
          variant="outline"
          onPress={onMore}
      ...
      {expanded && (
        <Box className="flex-row flex-wrap gap-2 px-4 pb-4">
          <Button
            ...
            accessibilityLabel={
              routine.archived ? `Un-archive ${routine.name}` : `Archive ${routine.name}`
            }
          >
            <ButtonText>
              {archiveBusy ? 'Saving…' : routine.archived ? 'Un-archive' : 'Archive'}
            </ButtonText>
          </Button>
          <Button
            variant="destructive"
