			...
			<Box className="flex-1 justify-end bg-foreground/40">
				...
				<Box className="max-h-[85%] gap-4 rounded-t-xl bg-popover px-4 pb-8 pt-4">
					<Box className="flex-row items-center justify-between gap-2">
						<Text size="2xl" className="font-semibold">Cannot delete Routine</Text>
					...
					{panel && (
						<ScrollView contentContainerClassName="gap-4" keyboardShouldPersistTaps="handled">
							<Text className="text-muted-foreground">{panel.routineName} still has references that must remain intact.</Text>
		...
		<Box className="gap-2">
			...
			{references.length === 0 ? (
				...
				<Box key={`${title}-${reference.id}`} className="rounded-xl bg-muted px-4 py-2">
					<Text size="sm">{title === 'Session history' && reference.date !== undefined ? `${reference.name} · ${referenceDate(reference.date)}` : reference.name}</Text>
				</Box>
			))}
		</Box>
	);
