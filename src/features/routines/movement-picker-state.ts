...
export function getMovementPickerRows(
	...
	return options
		.filter((movement) => {
			const matchesName = normalizedQuery === '' || movement.name.toLowerCase().includes(normalizedQuery);
			const matchesGroup = normalizedMuscleGroup === ALL_MUSCLE_GROUPS || movement.muscleGroup === normalizedMuscleGroup;
		...
		.sort((left, right) => {
			const nameOrder = left.movement.name.toLowerCase().localeCompare(right.movement.name.toLowerCase());
			return nameOrder || left.movement.id - right.movement.id;
		});
}

export function canSelectMovement(movement: CatalogMovement, selectedIds: readonly number[]): boolean {
