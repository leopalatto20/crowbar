  ...
  return (
    <Box className={`rounded-xl bg-card ${movement.archived ? 'opacity-50' : ''}`}>
      <Box className="flex-row items-center gap-2 px-4 py-4">
        ...
        {movement.archived && (
          <Text size="xs" bold className="text-muted-foreground uppercase">
            Archived
          </Text>
        )}
      </Box>
      {expanded && (
        <Box className="flex-row flex-wrap items-center gap-2 px-4 pb-4">
          <Button
            ...
            accessibilityLabel={
              movement.archived ? `Un-archive ${movement.name}` : `Archive ${movement.name}`
            }
          >
            <ButtonText>
              {archiveBusy ? 'Saving…' : movement.archived ? 'Un-archive' : 'Archive'}
            </ButtonText>
          </Button>
          <Button
            variant="destructive"
          ...
          {deleteBlocked && (
            <Button
              variant="outline"
              onPress={onShowReferences}
              accessibilityLabel={`Show references for ${movement.name}`}
            >
