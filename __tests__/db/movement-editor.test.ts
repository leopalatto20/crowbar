...
describe('movement editor repository seam', () => {
	...
	test('rejects exact duplicates, including archived movements', async () => {
		...
		const chest = (await listMuscleGroups(db)).find((group) => group.name === 'Chest');
		if (!chest) throw new Error('Chest was not seeded');
		const movement = await createMovement(db, { name: 'Archived Duplicate', primaryMuscleGroupId: chest.id });
		await db.update(schema.movements).set({ archived: 1 }).where(eq(schema.movements.id, movement.id));
		const near = await createMovement(db, { name: 'Archived Near Match', primaryMuscleGroupId: chest.id });
	...
	test('allows a self rename while preserving display spelling', async () => {
		...
		const chest = (await listMuscleGroups(db)).find((group) => group.name === 'Chest');
		if (!chest) throw new Error('Chest was not seeded');
		const movement = await createMovement(db, { name: 'Self Rename', primaryMuscleGroupId: chest.id });
	...
	test('finds exact names and one deterministic near match within distance two', async () => {
		...
		expect((await findMovementByName(db, 'ZyxAlph', { near: true, excludeId: first.id }))?.id).toBe(second.id);
	...
	test('updates each movement field independently and re-tags historical volume', async () => {
		...
		const movement = await createMovement(db, { name: 'Editable Movement', primaryMuscleGroupId: chest.id });
		const collision = await createMovement(db, { name: 'Collision Movement', primaryMuscleGroupId: chest.id });
		await expect(updateMovement(db, movement.id, { name: ` ${collision.name} ` })).rejects.toThrow(/already exists|duplicate/i);
		...
		await expect(updateMovement(db, movement.id, { name: 'Renamed Movement' })).resolves.toMatchObject({ name: 'Renamed Movement' });
		await expect(updateMovement(db, movement.id, { primaryMuscleGroupId: back.id })).resolves.toMatchObject({ primaryMuscleGroupId: back.id });
		await expect(updateMovement(db, movement.id, { unit: 'lb' })).resolves.toMatchObject({ unit: 'lb' });
		await expect(updateMovement(db, movement.id, { instructions: 'Brace and pause.' })).resolves.toMatchObject({ instructions: 'Brace and pause.' });
